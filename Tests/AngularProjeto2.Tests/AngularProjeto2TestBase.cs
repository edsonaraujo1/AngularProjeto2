...
using AngularProjeto2.Migrations.SeedData;
...
namespace AngularProjeto2.Tests
{
    public abstract class AngularProjeto2TestBase : AbpIntegratedTestBase<AngularProjeto2TestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected AngularProjeto2TestBase()
        {
        ...
        protected void UsingDbContext(Action<AngularProjeto2DbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<AngularProjeto2DbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<AngularProjeto2DbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<AngularProjeto2DbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<AngularProjeto2DbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<AngularProjeto2DbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<AngularProjeto2DbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<AngularProjeto2DbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<AngularProjeto2DbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<AngularProjeto2DbContext, Task<T>> func)
        {
            T result;
